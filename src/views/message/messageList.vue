      ...
      <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
        <el-tab-pane name="reply">
          <template #label>
            <div class="tab-item">
              <div>回复我的</div>
              <div class="messgaeCount" v-if="messageCount.reply>0">{{messageCount.reply>99?'99+':messageCount.reply}}</div>
            </div>
          </template>
        </el-tab-pane>
        <el-tab-pane  name="likePost">
          <template #label>
            <div class="tab-item">
              <div>赞了我的文章</div>
              <div class="messgaeCount" v-if="messageCount.likePost>0">{{messageCount.likePost>99?'99+':messageCount.likePost}}</div>
            </div>
          </template>
        </el-tab-pane>
        <el-tab-pane  name="downloadAttachment">
          <template #label>
            <div class="tab-item">
              <div>下载了我的附件</div>
              <div class="messgaeCount" v-if="messageCount.downloadAttachment>0">{{messageCount.downloadAttachment>99?'99+':messageCount.downloadAttachment}}</div>
            </div>
          </template>
        </el-tab-pane>
        <el-tab-pane  name="likeComment">
          <template #label>
            <div class="tab-item">
              <div>赞了我的评论</div>
              <div class="messgaeCount" v-if="messageCount.likeComment>0">{{messageCount.likeComment>99?'99+':messageCount.likeComment}}</div>
            </div>
          </template>
        </el-tab-pane>
          ...
          <template #default="{data}">
              <div class="reply" v-show="activeName=='reply'">
                  ...
                  <div style="padding: 10px 0px;margin-left: 5px;">
                    <div>
                    <span><a class="a-link" :href="'/userCenter/'+data.sendUserId">@{{ data.sendNickName }}</a></span>对我的文章【<a class="a-link" :href="'/post/'+data.articleId">{{data.articleTitle}}</a>】发表了评论 
                ...
                <Avatar :userId="data.sendUserId" ></Avatar>
                  <div style="padding: 10px 0px;margin-left: 5px;">
                    <div style="margin-top: 13px;">
                      <span><a class="a-link" :href="'/userCenter/'+data.sendUserId">@{{ data.sendNickName }}</a></span>赞了我的文章【<a class="a-link" :href="'/post/'+data.articleId">{{data.articleTitle}}</a>】
                      <span style="color: #9ba7b9;margin-left: 10px;">{{data.createTime}}</span> 
                  </div>
                  </div>
              </div>
              <div class="reply" v-show="activeName=='downloadAttachment'">
                ...
                <div style="padding: 10px 0px;margin-left: 5px;">
                  <div style="margin-top: 13px;">
                      <span><a class="a-link" :href="'/userCenter/'+data.sendUserId">@{{ data.sendNickName }}</a></span>在文章【<a class="a-link" :href="'/post/'+data.articleId">{{data.articleTitle}}</a>】下载了附件
                      <span style="color: #9ba7b9;margin-left: 10px;">{{data.createTime}}</span> 
                  </div>
                </div>
              </div>
              <div class="reply" v-show="activeName=='likeComment'">
                  ...
                  <div style="padding: 10px 0px;margin-left: 5px;">
                    <div>
                    <span><a class="a-link" :href="'/userCenter/'+data.sendUserId">@{{ data.sendNickName }}</a></span>在文章【<a class="a-link" :href="'/post/'+data.articleId">{{data.articleTitle}}</a>】赞了我的评论 
                   <span style="color: #9ba7b9;margin-left: 10px;">{{data.createTime}}</span> 
                  </div>
                  <div style="margin-top: 5px;">
                    <div class="replyContent">{{ data.messageContent }}</div>
              ...
              <div class="reply" v-show="activeName=='sys'">
                <div class="replyContent">{{ data.messageContent }}<span style="margin-left: 15px;color:#9ba7b9 ;">{{data.createTime}}</span></div>
              </div>
          </template>
        </DataList>
      </div>

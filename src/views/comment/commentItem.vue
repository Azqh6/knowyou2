<template>
    <div class="commentLists">
        ...
        <div class="comments">
            ...
            <div class="userInfo">
                ...
                <div class="commentTime">
                    ...
                    <span :class="['iconfont icon-good', commentData.likeType == 1 ? 'like' : '']"
                        @click="dolike(commentData)">&nbsp;{{ commentData.goodCount }}</span>
                    <span class="iconfont icon-comment"
                        @click="replyComment(commentData.userId, commentData.commentId)">&nbsp;回复</span>
                    <span v-show="store.state.loginUserInfo !== null && store.state.loginUserInfo.userId == userId"
                        class="makeTop">置顶</span>
                </div>
        ...
        <div class="replyUser" v-if="commentData.children !== null" v-for="(item, index) in commentData.children">
            ...
            <div class="user">
                <div class="top">
                    <span style="color: #61666d;margin-right: 10px;">{{ item.nickName }}</span>回复&nbsp;<span
                        style="color: var(--hoverColor);">@{{ item.replyNickName }}</span>：<span
                        style="font-size: 16px;">{{ item.content }}</span>
                </div>
                <div class="bottom">
                    <span style="color: #61666d;margin-right: 10px;">{{ item.postTime }}&nbsp;&nbsp;· {{ item.userIpAddress
                    }}</span>
                    <span :class="['iconfont icon-good', item.likeType == 1 ? 'like' : '']" @click="dolike(item)">&nbsp;{{
                        item.goodCount }}</span>
